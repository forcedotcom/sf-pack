...
exports.SfTasks = exports.SfJobInfo = void 0;
...
const sf_client_1 = require("./sf-client");
const sf_ui_1 = require("./sf-ui");
class SfJobInfo {
    constructor() {
        this.statusCount = 0;
...
class SfTasks {
    static async describeMetadata(org) {
        ...
        return !response?.metadataObjects ? [] : (0, ts_types_1.ensureArray)(response.metadataObjects);
    }
    static async *getTypesForPackage(org, describeMetadatas, namespaces = null) {
        ...
        for (const describeMetadata of describeMetadatas) {
            const members = [];
            try {
                sf_ui_1.SfUI.writeMessageCallback(describeMetadata.xmlName);
                if (!describeMetadata.inFolder) {
                    ...
                    const folderMetaName = describeMetadata.xmlName === sf_core_1.SfCore.EMAIL_TEMPLATE_XML_NAME
                        ? sf_core_1.SfCore.EMAIL_TEMPLATE_XML_NAME
                ...
                sf_ui_1.SfUI.writeMessageCallback(`ERROR: ${JSON.stringify(err)}`);
            }
    ...
    static async enqueueApexTests(org, sfEntities, shouldSkipCodeCoverage = false) {
        ...
        const client = new sf_client_1.SfClient(org);
        const uri = `${await client.getBaseUri(sf_client_1.ApiKind.TOOLING)}runTestsAsynchronous/`;
        // NOTE: If this returns 500 the daily limit may have been reached
        const result = await client.doAction(utils_2.RestAction.POST, uri, body);
        if (!result.id) {
    ...
    static async getBulkJobStatus(org, jobInfo) {
        ...
        const result = await client.doAction(utils_2.RestAction.GET, uri);
    ...
    static async getOrgInfo(org) {
        ...
        const authInfo = await core_1.AuthInfo.create({ username: org.getUsername() });
    ...
    static async getMaxQueryLimit() {
        return Number(await SfTasks.getConfigValue(constants_1.default.SF_CONFIG_MAX_QUERY_LIMIT));
    }
    static async getDefaultOrgAlias() {
        return SfTasks.getConfigValue(constants_1.default.SF_CONFIG_DEFAULT_USERNAME);
    }
    ...
    static async executeAnonymousBlock(org, apex) {
        ...
        // req.setMethod('GET');
        const client = new sf_client_1.SfClient(org);
        const uri = `${await client.getBaseUri(sf_client_1.ApiKind.TOOLING)}executeAnonymous/?anonymousBody=${encodeURI(apex)}`;
        // NOTE: If this returns 500 the daily limit may have been reached
        const result = await client.doAction(utils_2.RestAction.GET, uri);
        if (!result.isError) {
    ...
    static async getOrgLimits(org) {
        ...
        const client = new sf_client_1.SfClient(org);
        const uri = `${await client.getBaseUri(sf_client_1.ApiKind.LIMITS)}`;
        const result = await client.doAction(utils_2.RestAction.GET, uri);
        return result;
    }
    static async getFolderSOQLData(org) {
        if (!this.proFolderPaths) {
            const allFolders = await sf_query_1.SfQuery.getFolders(org);
...
exports.SfTasks = SfTasks;
SfTasks.defaultMetaTypes = [
    'ApexClass',
    'ApexPage',
    'CustomApplication',
    'CustomObject',
