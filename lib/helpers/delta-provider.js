...
class DeltaProvider {
        ...
        if (filePath && fullCopyDirNames) {
            ...
            for (const pathPart of pathParts) {
                if (gotFullCopyPath) {
                    ...
                    const newPathPart = pathPart.endsWith(constants_1.default.METADATA_FILE_SUFFIX) ? pathPart.split('.')[0] : pathPart;
            ...
            for await (const delta of this.diff(source)) {
                ...
                switch (deltaKind) {
                    ...
                    case DeltaProvider.deltaTypeKind.D:
                        ...
                        if (deleteReportFile) {
                            await node_fs_1.promises.appendFile(deleteReportFile, deltaFile + constants_1.default.EOL);
                        }
                        metrics.Del++;
                        break;
                    ...
                    case DeltaProvider.deltaTypeKind.M: {
                        ...
                        for await (const parentFilePath of utils_1.default.getFiles(parentDirName, false)) {
                            ...
                            if (path.basename(parentFilePath).startsWith(deltaParentBaseName) &&
                                parentFilePath.endsWith(constants_1.default.METADATA_FILE_SUFFIX)) {
    ...
    async logMessage(message, includeConsole = false) {
        if (typeof message === 'string') {
            await node_fs_1.promises.appendFile(this.logFile, `${message}${constants_1.default.EOL}`);
        }
        else {
            await node_fs_1.promises.appendFile(this.logFile, `${JSON.stringify(message)}${constants_1.default.EOL}`);
        }
