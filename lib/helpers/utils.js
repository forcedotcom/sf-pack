...
export class DefaultOptions {
    ...
    record_delimiter = Constants.CR + Constants.LF;
}
...
export default class Utils {
    ...
    static async getRestResult(action, url, parameter, headers, validStatusCodes, isFollowRedirects = true) {
        ...
        do {
            ...
            try {
                ...
                if (result.code === NO_CONTENT_CODE) {
                    return result;
                }
                // Read payload
                /* eslint-disable-next-line camelcase */
                response.content_type = response.headers[Constants.HEADERS_CONTENT_TYPE];
                const typeParts = response.content_type?.split('/');
                const mainType = typeParts[0];
                const subType = typeParts.length > 1 ? typeParts[1].split(';')[0] : null;
                switch (mainType) {
                    case 'application':
                    case 'image':
                    case 'audio':
                    case 'video':
                        if (subType === 'json') {
                            ...
                            result.body = await Utils.getBinaryResponse(response);
    ...
    static async getBinaryResponse(response) {
            ...
            exec(cmd, Utils.execOptions, (error, stdout) => {
                ...
                try {
                    if (stdout && String(stdout) !== '') {
                        ...
                        try {
                            response = JSON.parse(strippedResult);
                        }
                        catch {
    ...
    static async writeCSVFile(csvFilePath, data, csvOptions = new DefaultOptions()) {
