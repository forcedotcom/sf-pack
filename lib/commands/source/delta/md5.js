    ...
    async *diff(source) {
        ...
        if (hasUpdates) {
            ...
            if (!(await utils_1.default.pathExists(md5FilePath))) {
                ...
                await node_fs_1.promises.unlink(md5FilePath);
            }
            for (const [fp, data] of this.deltas) {
                await node_fs_1.promises.appendFile(md5FilePath, `${fp}${this.deltaLineToken}${data.hash}${constants_1.default.EOL}`);
            }
            await this.logMessage(`Updated hash file: ${md5FilePath} with ${this.deltas.size} entries.`, true);
        }
    }
};
Md5.flags = delta_command_1.DeltaCommandBase.getFlagsConfig({
    md5: sf_plugins_core_1.Flags.file({
        char: 'm',
