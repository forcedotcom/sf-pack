...
export default class Build extends CommandBase {
    ...
    async getMetadataMapFromOrg(options, flags) {
        ...
        if (flags.source) {
            ...
            let counter = 0;
            SfUI.writeMessageCallback = (message) => {
    ...
    async runInternal() {
        ...
        const packageFileName = flags.package || Constants.DEFAULT_PACKAGE_PATH;
        const folder = flags.folder;
        const packageDir = path.dirname(folder ? folder : packageFileName);
