...
export default class Execute extends CommandBase {
    ...
    static description = CommandBase.messages.getMessage('apex.coverage.execute.commandDescription');
    static examples = [
    ...
    async runInternal() {
        ...
        for await (recordCount of SfQuery.waitForApexTests(this.org, results.body, waitCountMaxSeconds)) {
            if (recordCount === 0) {
                break;
            }
