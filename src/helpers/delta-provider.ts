      ...
      for await (const delta of this.diff(source)) {
        ...
        switch (deltaKind) {
          ...
          case DeltaProvider.deltaTypeKind.M: {
            ...
            // If the file is a botVersion, we must ensure the parent bot file is included
            if (deltaFile.endsWith('.botVersion-meta.xml')) {
              const botDir = path.dirname(deltaFile);
              const botName = path.basename(botDir);
              const anchorFile = path.join(botDir, `${botName}.bot-meta.xml`);
              if (ignoreSet.has(anchorFile)) {
                await this.logMessage(`Delta (${deltaKind}) ignored Bot anchor: ${anchorFile}`, true);
                metrics.Ign++;
              } else if (!copiedSet.has(anchorFile) && await Utils.pathExists(anchorFile)) {
                const anchorDest = anchorFile.replace(source, destination);
                if (!isDryRun) {
                  await Utils.copyFile(anchorFile, anchorDest);
                }
                await this.logMessage(`Delta (${deltaKind}) auto-included Bot anchor: ${anchorDest}`);
                metrics.Copy++;
                copiedSet.add(anchorFile);
              }
            }
            ...
            for await (const parentFilePath of Utils.getFiles(parentDirName, false)) {
              ...
              if (
                path.basename(parentFilePath).startsWith(deltaParentBaseName) &&
