...
export default class Report extends CommandBase {
  ...
  protected async runInternal(): Promise<void> {
    ...
    for (const record of records) {
      sheetData.push([
        record.ApexClass?.Name as string,
        record.MethodName as string,
        record.Message as string,
        record.StackTrace as string,
        record.AsyncApexJobId as string,
        record.ApexTestRunResultId as string,
        record.TestTimestamp as string,
      ]);
    }
    workbookMap.set('Apex Test Failures', sheetData);

    const reportPath: string = flags.report || Report.defaultReportPath.replace(/\{ORG\}/, this.orgAlias);
